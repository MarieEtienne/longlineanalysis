...
#' 
#' @export
#' @return a vetor estimates of hat.lambdaT, hat.lambda.NT, hat.sigma

SEM.MLE <- function(llData, sameN=TRUE, sameS=TRUE, SameSigma=T, verbose=F) 
{
  ...
  res<- lapply(levels(llData$Fact1), function(d){
    ...
    tt <-SEM.MLE.YA(subdata=subdata, sameN=TRUE, sameP=TRUE, SameSigma=T, verbose=F)
    return(tt)
  })
...
SEM.MLE.YA <- function(subdata, sameN=TRUE, sameS=TRUE, SameSigma=1, verbose=F) {
